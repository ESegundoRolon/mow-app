...
public class LawnFileValidator {
	...
	public static void validateLawnDimension(final String[] tokens) {
		if (tokens == null || tokens.length != 2) {
			throw new InvalidFileFormatException("Lawn dimension should have 2 dimensions");
		}
	}
	...
	public static void validateMower(final String[] tokens, final LawnDimension dimension) {
		...
		if (tokens.length != 3 || dimension == null) {
			throw new InvalidFileFormatException("Mower position line is not valid or dimension is null");
		}
		...
		if (x > dimension.getX() || y > dimension.getY()) {
			throw new InvalidFileFormatException("Mower is specified out of the lwn boundaries");
		}
	}
